...
local images = { -- basically a image default
    menu = {}, -- all the images that should be displayed when the menu is shown
    sub_menu = {}, -- all the images that should be displayed when the game is in the sub menu
}
...
function love.load()
    ...
    images.menu.background = love.graphics.newImage("assets/background_menu.png");

    buttons.menu_state.play_game = button("Play Game", changeState, States.sub_menu, 300, 80) -- play game button
    buttons.menu_state.quit_game = button("Quit", love.event.quit, nil,300, 80) -- play game button
...
function love.draw()
    love.graphics.print("tam tela: " .. love.graphics.getWidth() .. "x" .. love.graphics.getHeight(), 10, 10)
    ...
    if game.state == States.sub_menu then
        ...
        buttons.sub_menu_state.back:draw(10, love.graphics.getHeight() - 50)
    ...
    elseif game.state == States.menu then -- if we're at the menu, draw the buttons
        love.graphics.draw(images.menu.background, 0, 0)
        -- draw the buttons in the mid of window
        buttons.menu_state.play_game:draw(love.graphics.getWidth() / 2 - buttons.menu_state.play_game:getWidth() / 2, love.graphics.getHeight() / 2 - buttons.menu_state.play_game:getHeight() / 2 - 50)
        buttons.menu_state.quit_game:draw(love.graphics.getWidth() / 2 - buttons.menu_state.quit_game:getWidth() / 2, love.graphics.getHeight() / 2 - buttons.menu_state.quit_game:getHeight() / 2 + 50)
